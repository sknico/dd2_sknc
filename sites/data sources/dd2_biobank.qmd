...
<!-- O:\\HE_KEA-DATA-RAW0050\\Main Part\\data\\Input Data Sets\\BioBank --> O:\\HE_KEA-DATA-RAW0050\\DD2 data\\Main Part - Local DD2 database\\data\\Input Data Sets\\BioBank

Data files from the 2022/2023 data updates are stored in the folder:\
<!-- O:\\HE\\KEA-DATA-RAW0050\\Biomarkører --> O:\\HE_KEA-DATA-RAW0050\\DD2 data\\Biomarkører
...
It is recommended not to use c-peptide measurements from this file, since the samples are analysed using an old type of analysis kit ([see below](dd2_biobank.qmd#c-peptide-and-glucose)). <!-- (e-mail fra JSN 31.05.2018)  --> <!-- "c-peptid --> <!-- Du har fortalt mig det før, men jeg skal bare lige være sikker.  --> <!-- Vi har c-peptid fra 3 forskellige datasæt (filer Klára har indlæst i databasen, og som jeg går ud fra hun har fået fra dig/DD2).  --> <!-- 1. First1053Patients: datasæt med blodprøvesvar på de første 1053 patienter. Enheden er pmol/L. Denne del børe vi aldrig bruge, De er på et gammelt kit og alle er kørt igen med den nye metode --> <!-- 2. DataTilDD2medFastende: data med ekstra biobank variable (c-peptid, GAD og glucose) samt en fastevariabel. Der er ca. 6000 patienter i dette datasæt. Der er ikke angivet nogen enhed i datasættet. Det er dette datasæt vi skal bruge, enheden er som in den game pmol/L.  --> <!-- 3. WrongCPeptideMeasurements: data med ca. 100 c-peptid resultater. Der er ikke angivet nogen enhed i datasættet. Dette data sæt brugte vi til at finde ud af at det nye og gamle kit ikke målte det samme, i kan beholde dem, men de giver som sådan ikke noget mening epidemiologisk.  --> <!-- 4. Når IDA inklussionen lukker her til sommer kommer der et ekstra data sæt med (C-pep, GAD og FB) dette vil for hovedparten være på patinter som ikke er med i de 6000 vi pt anvender. Altså øger vi antallet. Der vil være enkelte genmålinger af de første 6000 er der divergens mellem de to gemmes begge værdier med note. " -->

Information about some of the variables can be found in @Mor_DiabMetabResRev_2014.

-   `UrinResultatermedRatio2013Nov.txt`: DCE likely received the data file in December 2013. It includes N=1,053 CPR numbers (same as in `First1053Patients`) with results from the initial urine samples. The dataset includes the variables:
...
-   `WrongCPeptideMeasurements.txt`: The file appears to have been received in November 2015. It includes N=105 ID numbers (end with -00) and the variables cpeptid and NyCpeptid. The analysis was performed to compare values from different analysis kits, and the values were quite different. It is recommended not to use any of the measurements from this file. <!-- We have noted in the syntax that JSN has told us not to use these variables (e-mail 31.05.2018). -->
...
Please note that the unit for c-peptide has changed from pmol/L to mmol/L. <!-- (it is just the old unit divided by 1000). -->
...
## "Pladebiomarkører" {#pladebiomarkor}
...
An additional document combining overview sheets, method descriptions, and quality logs from some of the analysis rounds can be downloaded here: {{< downloadthis ../../files/biobank/Combined.pdf dname="Combined_pladebio" icon="file-earmark-pdf" label="Download" type="button" class="btn btn-link">}} <!-- I nogle af dokumenterne har jeg fjernet information der omhandler et enkelt id-nummer -->
